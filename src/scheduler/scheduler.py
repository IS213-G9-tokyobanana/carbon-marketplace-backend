...
from config.rabbitmq_setup import RMQHOSTNAME, RMQPASSWORD, RMQPORT, RMQUSERNAME
...
def schedule_jobs(message: dict):
    ...
    if type == MessageType.MILESTONE_ADD.value:
        ...
        type == MessageType.OFFSETS_COMMIT.value
        or type == MessageType.PAYMENT_FAILED.value
    ):
        ...
        type == MessageType.MILESTONE_REWARD.value
        or type == MessageType.MILESTONE_PENALISE.value
    ):
        ...
        type in TaskType.values()
    ):  # elif the type is one of the published task type that supervisor publishes back
...
def create_track_payment_job(payment_id, created_at, project_id, milestone_id):
    ...
    job = cron.new(
        command=f"{PYTHON_EXE} {PUBLISHER_SCRIPT} --type {type} --project {project_id} --milestone {milestone_id} --payment {payment_id}",
        comment=f"{type}_{payment_id}",
    )
...
def create_milestone_job(milestone: dict, job_type: TaskType):
    if job_type == TaskType.MILESTONE_UPCOMING.value:
        ...
        cron = CronTab(user=True)
        job = cron.new(
            command=f"{PYTHON_EXE} {PUBLISHER_SCRIPT} --type {type} --milestone {milestone_id} --project {project_id}",
            comment=f"{type}_{milestone_id}",
        )
        ...
        # test_time = datetime.utcnow() + timedelta(
        #     minutes=1
