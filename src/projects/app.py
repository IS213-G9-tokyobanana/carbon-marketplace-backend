...
from classes.enums import AmqpPayloadType, MilestoneStatus, ProjectStatus
...
from models.models import Milestone, Project, ReservedOffset, db
...
def create_project():
    ...
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_CREATED_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def update_project_status(id):
    ...
    payload_serialised = json.dumps(payload.json()) 
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_VERIFIED_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def create_project_milestone(id):
    ...
    payload_serialised = json.dumps(payload.json())
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_ADD_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def update_project_milestone_status(id, mid):
    ...
    if body['status'] == MilestoneStatus.MET.value:
        ...
        payload_serialised = json.dumps(payload.json()) 
        publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_REWARD_QUEUE][ROUTING_KEY], message=payload_serialised)
    ...
    # decrease project rating + publish to project_penalised queue
        ...
        payload_serialised = json.dumps(payload.json()) 
        publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_PENALISE_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def create_reserved_offset(id, mid):
    ...
    try:
        ...
        abort(400, description=f"Invalid quantity_tco2e {quantity_tco2e}. quantity_tco2e must be a number.")

    if quantity_tco2e > milestone.offsets_available:
        abort(400, description=f"Unable to reserve {quantity_tco2e} offsets. quantity_tco2e to reserve {quantity_tco2e} exceeds offsets available {milestone.offsets_available}.")
    
    # Insert new row in reserved offset table to reserve the quantity_tco2e, user_id, milestone_id, for a payment_id.
    reserved_offset = ReservedOffset(**body, milestone_id=str(mid))
    db.session.add(reserved_offset)

    # Deduct `quantity_tco2e` from request body from project milestone to reserve the quantity_tco2e for the buyer
    milestone.offsets_available -= quantity_tco2e
    ...
    payload_serialised = json.dumps(payload.json())
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_RESERVE_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def commit_reserved_offset():
    ...
    payload_serialised = json.dumps(payload.json())
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_COMMIT_QUEUE][ROUTING_KEY], message=payload_serialised)
...
def rollback_reserved_offset():
    ...
    payload_serialised = json.dumps(payload.json())
    publish_message(connection=connection, channel=channel, exchangename=exchangename, routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_ROLLBACK_QUEUE][ROUTING_KEY], message=payload_serialised)
