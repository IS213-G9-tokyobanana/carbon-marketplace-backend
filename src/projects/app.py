...
def create_project():
    ...
    for milestone in milestones:
        milestone["due_date"] = datetime.strptime(
            milestone["due_date"], request_time_format
        )  # parse iso format string in request body to datetime
        m = Milestone(
            **milestone, project=project, offsets_total=milestone["offsets_available"]
        )
        db.session.add(m)
    ...
    }  # project will have all the milestones populated due to the `relationship()` in models.py
    payload = Payload(
        resource_id=str(project.id),
        type=AmqpPayloadType.PROJECT_CREATE.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_CREATED_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
...
def update_project_status(id):
    ...
    if body["status"] not in ProjectStatus.values():
        abort(
            400,
            description=f"Invalid status '{body['status']}'. Status can only be the following: {ProjectStatus.values()}",
        )
    ...
    payload = Payload(
        resource_id=str(project.id),
        type=AmqpPayloadType.PROJECT_VERIFY.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_VERIFIED_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
...
def create_project_milestone(id):
    ...
    body["due_date"] = datetime.strptime(
        body["due_date"], request_time_format
    )  # parse iso format string in request body to datetime
    milestone = Milestone(
        **body, project=project, offsets_total=body["offsets_available"]
    )
    ...
    payload = Payload(
        ...
        type=AmqpPayloadType.MILESTONE_ADD.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_MILESTONES_ADD_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
...
def update_project_milestone_status(id, mid):
    ...
    if body["status"] not in MilestoneStatus.values():
        abort(
            400,
            description=f"Invalid status '{body['status']}'. Status can only be the following: {MilestoneStatus.values()}",
        )
    ...
    if body["status"] == MilestoneStatus.MET.value:
        ...
        publish_message(
            exchangename=exchangename,
            routing_key=QUEUES[PROJECTS_MILESTONES_REWARD_QUEUE][ROUTING_KEY],
            message=payload_serialised,
        )
    ...
    # decrease project rating + publish to project_penalised queue
        ...
        publish_message(
            exchangename=exchangename,
            routing_key=QUEUES[PROJECTS_MILESTONES_PENALISE_QUEUE][ROUTING_KEY],
            message=payload_serialised,
        )
...
def find_milestone_by_id(mid):
    milestone = (
        db.session.execute(db.select(Milestone).where(Milestone.id == mid))
        .scalars()
        .first()
    )
...
def create_reserved_offset(id, mid):
    ...
    if (
        db.session.execute(
            db.select(ReservedOffset).where(ReservedOffset.payment_id == payment_id)
        )
        .scalars()
        .first()
        is not None
    ...
    if amount > milestone.offsets_available:
        abort(
            400,
            description=f"Unable to reserve {amount} offsets. amount to reserve {amount} exceeds offsets available {milestone.offsets_available}.",
        )
    ...
    payload = Payload(
        resource_id=reserved_offset.payment_id,
        type=AmqpPayloadType.OFFSETS_RESERVE.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_RESERVE_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
...
def find_reserved_offset(pid):
    reservation = (
        db.session.execute(
            db.select(ReservedOffset).where(ReservedOffset.payment_id == pid)
        )
        .scalars()
        .first()
    )
...
def commit_reserved_offset():
    ...
    app.logger.info(f"body: {body}")
    payment_id = body["payment_id"]
    reservation: ReservedOffset or None = (
        db.session.execute(
            db.select(ReservedOffset).where(ReservedOffset.payment_id == payment_id)
        )
        .scalars()
    ...
    payload = Payload(
        resource_id=reservation.payment_id,
        type=AmqpPayloadType.OFFSETS_COMMIT.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_COMMIT_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
...
def rollback_reserved_offset():
    ...
    milestone = (
        db.session.execute(db.select(Milestone).where(Milestone.id == milestone_id))
        .scalars()
        .first()
    )
    ...
    amqp_data = {"reserved_offset": reservation.json()}
    project = (
        db.session.execute(db.select(Project).where(Project.id == milestone.project_id))
        .scalars()
        .first()
    )
    amqp_data["project"] = project.json()  # add project to the payload for search
    payload = Payload(
        resource_id=reservation.payment_id,
        type=AmqpPayloadType.OFFSETS_ROLLBACK.value,
        data=amqp_data,
    )
    payload_serialised = json.dumps(payload.json())
    publish_message(
        exchangename=exchangename,
        routing_key=QUEUES[PROJECTS_MILESTONES_OFFSETS_ROLLBACK_QUEUE][ROUTING_KEY],
        message=payload_serialised,
    )
