...
def find_all_users():
    ...
    if milestone_id is not None:
        offsets = db.session.execute(db.select(Offset).where(Offset.milestone_id == milestone_id)).scalars().all()
...
def update_buyer_offset(id, payment_id):
    ...
    buyer: User or None = db.session.execute(db.select(User).where(User.id == id)).scalars().first()
    if buyer is None:
        abort(404, description=f"User {str(id)} not found.")
    ...
    if status not in OffsetStatus.values():
        abort(400, description=f"Invalid status '{status}'. Status can only be the following: {OffsetStatus.values()}")

    offset = db.session.execute(db.select(Offset).where(Offset.payment_id == payment_id)).scalars().first()
    if offset is None:
